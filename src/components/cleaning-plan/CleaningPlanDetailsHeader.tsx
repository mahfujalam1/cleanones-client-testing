      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-base font-bold text-slate-800 leading-tight">{details.title || "Cleaning Plan"}</h2>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            ID: {details._id?.slice(0, 10)}{formattedDate ? ` • Created ${formattedDate}` : ""}
          </p>
        </div>
      </div>
      <div className="flex items-center gap-2">
        <button onClick={onBack} className="ml-2 p-1 text-slate-400 hover:text-slate-600 transition-colors" aria-label="Go back">
